...
directory you passed to :meth:`prettyresults.ResultTree.generate_web` and
...
- **Qualified ID**. A qualified ID uniquely identifies a result within a ResultTree.
...
- :code:`root`. This is the root of the result tree. It is implicitly added when the ResultTree
  gets created.
...
Note that these are unique across the entire ResultTree. Qualified IDs are employed,
for example, to retrieve results in :meth:`prettyresults.ResultTree.get_result`.
...
document may be impractical. You can use :meth:`prettyresults.ResultTree.generate_word`
